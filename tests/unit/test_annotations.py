...
import shutil
import pandas as pd
from oid2yolo.annotations import split_by_imageid
...
def test_split_by_imageid():
    ...
    df_all, df_rest = split_by_imageid(df)
    assert len(df_all) > 0
...
def test_extract_from_oid_bbox_with_class_names():
    label_filter = convert_label_filter(image_filter, df_all_classes)
    new_bbox_csv = extract_from_oid_bbox_with_class_names(oid_bbox_csv, label_filter, df_classes)
